...
const statusStyles = {
	...
	review: "bg-yellow-500/10 text-yellow-400 ring-1 ring-yellow-500/20",
			...
			<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
							...
							<tbody className="divide-y divide-white/5">
								{claims.map((claim) => (
									...
									>
										...
										<td className="px-6 py-4">
											<span
												className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${statusStyles[claim.status]}`}
											>
				...
				<div className="rounded-2xl bg-gray-900 ring-1 ring-white/10 overflow-hidden">
					...
					<ul className="divide-y divide-white/5">
						{activity.map((item) => (
							<li key={item.id + item.time} className="flex items-start gap-x-4 px-6 py-4">
								<span
			...
			<div className="rounded-2xl bg-gray-900 ring-1 ring-white/10 p-6">
				<div className="flex items-center justify-between mb-6">
					...
					<div className="flex items-center gap-x-4 text-xs text-gray-400">
						...
						<span className="flex items-center gap-x-1.5">
							<span className="size-2 rounded-full bg-yellow-500" /> Medium (75–89%)
						</span>
						<span className="flex items-center gap-x-1.5">
				...
				<div className="flex items-end gap-x-1.5 h-32">
					{[72, 85, 91, 78, 94, 88, 96, 63, 89, 97, 82, 91, 74, 88, 95, 79, 92, 86, 98, 71, 90, 83, 77, 93].map(
						(v, i) => {
							const color =
								v >= 90
									? "bg-green-500"
